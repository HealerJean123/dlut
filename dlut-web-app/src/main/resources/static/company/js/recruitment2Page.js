$(document).ready(function(){
	...
	function getContent(now){//分页功能
		$("#page").attr({"pagelistcount":$("#number").val()||10});
			$.ajax({//请求数据
			type:"get",
			url:"/app/company/queryByCondition",
			data:{
				name:$("[name=name]").val(),
			...
			success:function(obj){
				...
				if(obj.content==null || obj.content==""){
				    ...
				    $.each(obj.content, function(i,e) {
                        ...
                        inner +='<tr class="text-center"><td>'+$(e)[0].name+'</td><td>'+$(e)[0].typeName+'</td><td>'+$(e)[0].degreeName +'</td><td>'+$(e)[0].major+'</td><td>'+$(e)[0].city+'</td><td>'+$(e)[0].recruitmentNum+'</td><td>'+startTime+'</td><td>'+endTime+'</td><td>'+$(e)[0].auditStatusName+'</td><td><a href="/app/company/jobDetail?id='+$(e)[0].id+'">详情</a></td></tr>';
                	});
				}
