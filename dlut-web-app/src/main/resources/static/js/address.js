...
{  
    var cmbProvince = document.getElementById(_cmbProvince);  //找到省的输入框
    var cmbCity = document.getElementById(_cmbCity);//找到市的输入框  
    var cmbArea = document.getElementById(_cmbArea);  //找到县的输入框
      
    function cmbSelect(cmb, str)  
    {  
        for(var i=0; i<cmb.options.length; i++)  //循环所有的省份框的option元素，
        {  
            if(cmb.options[i].value == str)  //
            {  
                cmb.selectedIndex = i;  //记录这个省份的索引值
                return;  
            }  
        }  
    }  
    function cmbAddOption(cmb, str, obj)  //添加option的值
    {  
        var option = document.createElement("OPTION");  //创建标签option
        cmb.options.add(option);  //------增加------------------------------------------------？？？？？？？？？？？？？
        option.innerText = str;  //内容
        option.value = str;  //值
        option.obj = obj;  //对象
    }  
    ...
    {  
        cmbCity.options.length = 0;  //初始化
        cmbCity.onchange = null;  
        if(cmbProvince.selectedIndex == -1)return;  
        var item = cmbProvince.options[cmbProvince.selectedIndex].obj;  //找到市
    ...
    for(var i=0; i<provinceList.length; i++)  //循环省份的长度
    {  
        cmbAddOption(cmbProvince, provinceList[i].name, provinceList[i]);  //传入这个下拉框，下拉框应该显示的值，这一组的值
    }  
    cmbSelect(cmbProvince, defaultProvince);  //省份，默认省份
    changeProvince();  
