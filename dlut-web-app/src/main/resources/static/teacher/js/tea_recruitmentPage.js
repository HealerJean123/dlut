$(document).ready(function(){
	...
	function getContent(now){//分页功能
		$("#page").attr({"pagelistcount":$("#number").val()||10});
			$.ajax({//请求数据
			type:"get",
			url:"/app/teacher/teaFindBulletin",
			data:{
			...
			success:function(obj){
				...
				if(obj.content==null || obj.content==""){
				    ...
				    $.each(obj.content, function(i,e) {
                        ...
                        inner +='<tr class="text-center"><td><input type="checkbox" value="'+$(e)[0].id+'" class="checkbox" /></td><td>'+$(e)[0].recName+'</td><td>'+$(e)[0].title+'</td><td>'+$(e)[0].auditStatusName+'</td><td>'+publishTime +'</td><td>'+startTime+'</td><td>'+endTime+'</td><td><a class="btn btn-default" href="/app/teacher/teaBulletinDetail?id='+$(e)[0].id+'">详情</a></td><td><a class="a btn btn-default" data-val="'+$(e)[0].id+'">审核通过</a></td></tr>';
                	});
				}
    ...
    $("#batch").on("click",function(){
    	...
    	$.ajax({
    		type:"post",
    		url:"/app/teacher/teaMoreBulletinAudit",
    		data:{
    			"ids":box
    		},
    		traditional: true,
    ...
    function agree(id){
        $.ajax({
            type:"post",
            url:"/app/teacher/teaBulletinPass",
            data:{id:id},
            success:function(data){
    			$("#tc").find("#label").html(data);
    			$("#tc").modal("show")
